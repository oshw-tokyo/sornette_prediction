...
3. **境界張り付きチェック**（重要更新）
   - 境界張り付きはフィッティング失敗として扱う
   - 適格フィットから除外（Confidence計算に含めない）
   - 理由: 真の最適解の保証（探索範囲制約による人工的収束の排除）

4. **予測精度達成率測定**（追加要件）
   - 各窓の予測誤差≤35日達成率を補助的に測定
   - FCO総合結果の精度が最終的に重要
   - 報告内容: 適格フィット率・予測精度達成率の両方

#### 成功基準（更新版）
- ✅ 1987年ブラックマンデー: DS-LPPLS Confidence > 30%（FCO標準閾値）
- ✅ バブルタイプ: positive_bubble判定
- ✅ 全126窓でフィッティング正常動作（段階的検証は不要、最初から126窓）
- ✅ 境界張り付きのない適格フィット > 30%
- ✅ 予測精度達成率の測定・報告
...
| Phase | 作業内容 | 工数 | 成果物 |
|-------|---------|------|--------|
| **Phase 1** | 過去実装復元・単一窓検証 | 2-3日 | R² > 0.9、誤差≤35日、境界張り付きなし |
| **Phase 2** | 多重窓解析統合（126窓） | 1週間 | Confidence > 30%、精度達成率測定 |
| **Phase 3** | DB/フロントエンド統合 | 2-3日 | 完全統合動作確認 |
| **Phase 4** | 最終検証・最適化 | 3-5日 | 2000年・2008年段階的実装 |
| **合計** | - | **2-3週間** | カスタムFCO実装完成 |

**重要な更新**:
- Phase 1: 予測誤差基準を35日に厳格化、境界張り付きを失敗として扱う
- Phase 2: 段階的窓数検証（10, 50窓）を削除、最初から126窓で実装
- Phase 2: 予測精度達成率測定を追加（各窓の誤差≤35日達成率）
- Phase 4: 他の歴史的クラッシュは段階的に実装（1987年→2000年→2008年）
