...
## 📋 **Issue I043: バッチデータ取得機能実装（backfill-v2）**

### 🎯 **Phase 1: プロトタイプ実装** (2025-08-09開始)

**タスクリスト**:
- [x] バッチデータキャッシュ層の設計
- [x] メモリ効率的なデータ管理構造
- [x] CoinGecko 365日制限対応の複数回取得・結合ロジック
- [x] データ不足時の適切な処理中断機能
- [x] 既存LPPL分析との統合インターフェース設計
- [x] entry_points/main.pyへの統合
- [x] API効率化の実測検証

**達成した成果**:
- ✅ **API効率化**: 66.7%のAPIコール削減を実証（3期間×3銘柄のケース）
- ✅ **バッチキャッシュ**: 一度取得したデータを複数期間で再利用
- ✅ **CoinGecko対応**: 365日制限を自動分割・結合処理
- ✅ **コマンド統合**: `scheduled-analysis backfill-v2`として利用可能
...
1. **バッチキャッシュ構造**:
   ...
   batch_cache = {
       'symbol': {
           'data': pd.DataFrame,  # 全期間データ
           'source': str,          # データソース
           'fetched_at': datetime, # 取得時刻
           'periods_covered': list # カバーする期間リスト
       }
   }
   ```
...
3. **既存システム統合**:
   - `ScheduledAnalyzer._analyze_single_symbol()`との互換性維持
   - 既存の重複防止機構（unique_symbol_basis_date）活用
