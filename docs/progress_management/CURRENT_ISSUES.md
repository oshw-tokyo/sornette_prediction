...
| I057 | Crash Prediction Clustering Development タブ実装・信頼性向上実験 | 現在のクラスタリングタブ（I052）を保持し、開発版タブで信頼性向上実験：1)多次元パラメータクラスタリング仮説検証（ω周波数等）2)クラッシュ直前フィッティング除外フィルター（最小予測期間10日）3)プロット軸変更（縦横入替）4)R²重み付け根拠明確化・代替手法検討 | 2025-08-13 | クラスタリング精度・投資判断支援品質 | 🔄 実装中 |
...
| I053 | LPPLフィッティングタブの表示・切り替えバグ | Price Predictionsタブで複数の問題発生・【銘柄切り替え後プロットバグ】異なる銘柄を選択した際にプロット表示が破損・重複データや座標系の問題・【個別フィッティング結果未表示】LPPLフィッティング結果の詳細グラフが正常に表示されない・プロット生成エラーまたは空白表示・【期間フィルタリング後の表示問題】Display Period変更後の再描画で不整合・【SESSION_STATE管理問題】タブ間でのセッション状態干渉・Apply Period後の状態リセット問題・ダッシュボード実用性に重大な影響・早急なデバッグと修正が必要 | 2025-08-12 | ダッシュボード安定性・LPPL表示機能・ユーザビリティ | 🟠 High |
| I055 | Clustering Analysis Settings統一とApplyボタン制御 | Crash Prediction Clusteringタブのパラメータ制御改善・【Applyボタン統一】Display PeriodとClustering Parameters両方を単一Applyボタンで制御・【パラメータプレビュー】スライダー変更は即座に反映せずApply押下時に適用・【設定グループ化】Clustering Analysis Settingsとして全設定を統合・【ラベル改善】Display Period→Analysis Data Period（解析データの対象期間）に変更・設定値のプレビューと実際の適用値を明確に区別・ユーザー操作の一貫性向上 | 2025-08-13 | ダッシュボードUX・パラメータ管理 | 🟡 Medium |
| I058 | LPPL Fitting Analysis Individual Results UI改善・歴史的検証統合計画 | LPPL Fitting Analysisタブの【Individual Fitting Results表示問題】現在の表示が使いにくい状態・解析済みデータとプロット・生データの統合確認による分析結果妥当性の直感的検証機能の実現・【将来的統合目標】同UIで種々の解析結果確認と共に、ブラックマンデー等の特定クラッシュも表示可能とする・本プロジェクトプロット妥当性の可視化実現・UI設計段階だが、科学的妥当性検証とデータ確認機能の統合により、実用性と学術的価値を両立させる重要機能 | 2025-08-13 | ダッシュボードUX・科学的検証・実用性 | 🟡 Medium |
