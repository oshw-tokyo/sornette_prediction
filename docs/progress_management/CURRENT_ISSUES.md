...
### I136: 🔴 グリッドサーチ初期値生成の最適化（計算効率1/10目標）
**作成日**: 2025-10-13
**優先度**: 🔴 Critical
**担当**: パフォーマンス最適化チーム
**状態**: 🚧 調査・実装中
...
**目的**:
グリッドサーチの初期値生成方法を最適化し、計算回数を1/10以下に削減する。現在の計算時間（51窓×4基準日で約3時間）は実用的ではなく、実験サイクルを著しく阻害している。

**現状の問題点**:

1. **計算規模が過大**:
   ```
   現在: n_tries=8 → 8³ = 512組み合わせ/窓
   51窓 × 4基準日 × 512組み合わせ = 104,448回のフィッティング/テスト
   3テスト合計: 313,344回のフィッティング
   実行時間: 約3時間（途中停止）
   ```

2. **グリッドサーチ範囲が非効率**:
   - 現在のグリッドサーチ範囲 = bounds範囲全体
   - 境界値から開始 → 境界張り付きリスク増加
   - 全範囲を均等探索 → 無駄な探索が多い

3. **Boulder LPPLS FCOとの比較**:
   ```python
   # Boulder LPPLS (ランダム生成)
   max_searches = 25  # デフォルト（文献推奨値）
   init_limits = [
   ...
   # カスタムFCO (グリッドサーチ)
   n_tries = 8  # → 512組み合わせ
   tc_values = np.linspace(1.01, 1.5, 8)      # bounds全体
   beta_values = np.linspace(0.1, 0.9, 8)     # bounds全体
   omega_values = np.linspace(2.0, 15.0, 8)   # bounds全体
   ```

**Boulder LPPLS調査結果**:
- **ファイル**: `/home/no-rules/.local/lib/python3.10/site-packages/lppls/lppls.py` (lines 134-142)
- **初期値生成**: 完全ランダム（グリッドサーチではない）
- **試行回数**: 25回（512回の1/20）
- **omega範囲**: 6.0-13.0（bounds 2.0-15.0より狭い、中心部のみ探索）

**改善提案**:

### **Option A: Boulder LPPLS準拠（ランダム生成）** ← 🎯 推奨
```python
# 25回ランダム生成 → 1/20の計算量
n_tries = 25  # 512 → 25 (1/20)
for _ in range(n_tries):
    tc = random.uniform(1.059, 1.451)      # bounds内側10%-90%
    beta = random.uniform(0.18, 0.82)      # bounds内側10%-90%
    omega = random.uniform(3.3, 13.7)      # bounds内側10%-90%
```

**メリット**:
- ✅ 計算量: 512 → 25 (1/20削減、目標1/10達成)
- ✅ Boulder LPPLS標準準拠（実績あり）
- ✅ 境界張り付きリスク低減（内側のみ探索）
- ✅ 実装が簡単

**デメリット**:
- ⚠️ グリッドサーチの系統的探索を放棄
- ⚠️ 再現性が低下（ランダムシード管理必要）

### **Option B: 粗いグリッドサーチ + 内側範囲**
```python
# 3³ = 27組み合わせ
n_tries = 3  # 512 → 27 (1/19削減、目標1/10達成)
tc_values = np.linspace(1.059, 1.451, 3)      # bounds内側10%-90%
beta_values = np.linspace(0.18, 0.82, 3)      # bounds内側10%-90%
omega_values = np.linspace(3.3, 13.7, 3)      # bounds内側10%-90%
```

**メリット**:
- ✅ 計算量: 512 → 27 (1/19削減、目標1/10達成)
- ✅ 系統的探索を維持
- ✅ 完全な再現性
- ✅ 境界張り付きリスク低減
...
### **Option C: ハイブリッド（少数グリッド + ランダム補完）**
```python
# 2³ = 8 (グリッド) + 17 (ランダム) = 25組み合わせ
# グリッド: 系統的に主要領域をカバー
tc_grid = np.linspace(1.15, 1.35, 2)      # 中心部のみ
beta_grid = np.linspace(0.3, 0.7, 2)      # 典型値範囲
omega_grid = np.linspace(6.0, 10.0, 2)    # 典型値範囲

# ランダム: 周辺領域も探索
for _ in range(17):
    tc_rand = random.uniform(1.059, 1.451)
    beta_rand = random.uniform(0.18, 0.82)
    omega_rand = random.uniform(3.3, 13.7)
```
...
1. 🎯 **Option A実装を推奨** (Boulder LPPLS準拠、最もシンプル)
2. 📊 **1987年テストで効果測定**:
   - 計算時間: 約3時間 → 約9分（1/20）
   - DS-LPPLS Confidence変化測定
   - 予測精度変化測定
3. 📈 **効果確認後、全テストに適用**

**関連Issue**:
- Issue I130: Boulder LPPLS FCO調査（✅ 完了、初期値生成方法を確認）
- Issue I133: 歴史的クラッシュ再現性テスト（🚧 計算時間が課題）
- Issue I137: 計算ボトルネック総合調査（並列）

---

### I137: ✅ 計算ボトルネック総合調査（フィッティングプロセス高速化）
**作成日**: 2025-10-13
**優先度**: 🔵 Medium
**担当**: パフォーマンス最適化チーム
**状態**: ✅ 調査完了（実装見送り）

**⚠️⚠️⚠️ 調査結果: 効果が限定的のため実装見送り（2025-10-13）⚠️⚠️⚠️**

**調査結果サマリー**:
| 最適化手法 | 高速化倍率 | 実装コスト | 判定 |
|-----------|-----------|-----------|------|
| **I136 (ランダム初期値)** | **21.2x** | 低 | ✅ **実装済み** |
| I137-a (curve_fit緩和) | 1.04x | 低 | ❌ 効果微小 |
| I137-b (Numba JIT) | 0.88x | 中 | ❌ 逆効果 |
| I137-a + I137-b (組合せ) | 1.26x | 中 | ❌ 効果微小 |

**実測データ**（1987年、51窓、25試行/窓）:
- **ベースライン**: 28.24秒 (0.47分) - DS-LPPLS Confidence: 94.12%
- **期待値（I137-a+b）**: ~22.4秒 (0.37分) - **削減時間: わずか5.8秒**

**結論**:
1. ✅ **I136単独で目標達成**: 21.2倍高速化（計算時間: 約3時間 → 約9分）
2. ❌ **I137-a効果微小**: ftol/xtol/gtol緩和で1.04倍（ほぼ変化なし）
3. ❌ **I137-b逆効果**: Numba JITで0.88倍（オーバーヘッドで遅延）
4. ❌ **組合せも微小**: 1.26倍（約5.8秒削減のみ）
5. 🎯 **現在の計算時間は許容範囲**: 51窓で0.47分は十分高速

**実装見送り理由**:
- I136で既に1/20の計算量削減を達成（目標「1/10以上削減」を大幅達成）
- I137追加最適化の効果は約20%のみ（費用対効果が低い）
- 現在の計算時間（0.47分/51窓）は実用上問題なし
- 精度維持を優先し、無理な最適化は避ける方針

**テスト実装**:
- `workspace_for_claude/test_i137_optimizations.py` - 単一窓効果測定
- `workspace_for_claude/test_i137_full_fco.py` - 多重窓全体測定
- `logs/test_i137_optimizations.log` - 単一窓テスト結果
- `logs/test_i137_full_fco_baseline.log` - ベースライン測定結果

**目的**:
グリッドサーチ回数削減（I136）以外の高速化手法を調査し、フィッティングプロセス自体の高速化を実現する。

**背景**:
- Issue I127で並列化（multiprocessing）を実装済み（約8倍高速化）
- しかし、フィッティングプロセス自体（単一curve_fit呼び出し）の高速化は限定的
- 計算時間の大部分はscipy.optimize.curve_fitが占める

**調査項目（参考）**:

### 1. **curve_fit最適化パラメータの調整**
```python
# 現在の設定
popt, pcov = curve_fit(
...
**調査内容**:
- 許容誤差（ftol, xtol, gtol）の緩和: 1e-6 → 1e-4
- 最大評価回数（max_nfev）の削減: 50000 → 10000
- 最適化手法の変更: 'trf' → 'dogbox' or 'lm'
- **効果**: 収束速度向上、ただし精度トレードオフ
...
def logarithm_periodic_func(t, tc, beta, omega, phi, A, B, C):
    ...
    oscillation = B * power_law * np.cos(omega * log_dt + phi)
    damping = C * power_law * np.sin(omega * log_dt + phi)
    return A + oscillation + damping
```

**調査内容**:
- Numba JIT コンパイル（`@njit`デコレータ）
- Cython での実装
- ベクトル化の最適化
- **効果**: 関数評価の高速化（Boulder LPPLSはnumba使用）

### 3. **初期値推定の改善**
```python
# 現在: グリッドサーチの初期値をそのまま使用
p0 = [tc, beta, omega, 0.0, np.log(np.mean(log_prices)), B_init, 0.1]
```

**調査内容**:
- 線形パラメータ（A, B, C）の事前推定精度向上
- tc の粗推定（ピーク検出等）
- **効果**: 収束までの反復回数削減
...
# 提案: 窓サイズに応じて設定を調整
if window_size < 300:
    max_nfev = 10000  # 小窓は早期収束
else:
    max_nfev = 50000  # 大窓は精密最適化
```
...
**調査方法**:
1. 各手法の個別効果測定（1987年テスト、単一窓で比較）
2. 精度への影響評価（R²、tc精度）
3. 総合的なコスト・ベネフィット分析

**優先順位**:
1. 🔴 **高優先度**: I136（グリッドサーチ削減）← まずこれを実施
2. 🔵 **中優先度**: curve_fit最適化パラメータ調整
3. 🟢 **低優先度**: Numba JIT / Cython（実装コスト高）

**次のアクション**:
- Issue I136の実装完了を待つ
- I136の効果を測定後、追加最適化の必要性を判断

**関連Issue**:
- Issue I127: 並列化実装（✅ 完了、約8倍高速化達成）
- Issue I136: グリッドサーチ最適化（🚧 最優先）
...
**✅ Phase 1完了: O/D閾値実装 + 過剰フィルタリング不活性化**
- commit: e7ff57b
- 実装内容:
  1. `lppl_utils.py`: `calculate_oscillations()`, `calculate_damping()`, `convert_tc_to_date()` 追加
  2. `custom_fco_engine.py`: O > 2.5, D > 0.5 フィルタ追加
  3. tc範囲チェック・境界張り付きチェック一時無効化（フラグ制御）
- 検証結果（1987年ブラックマンデー）:
  - ❌ DS-LPPLS Confidence: 43.14% → 11.76%（大幅低下）
  - ❌ 適格フィット数: 21/51 → 6/51（71%減少）
  - ⚠️ 全6適格フィットで **omega=5.0（下限張り付き）**
  - ✅ 予測誤差: 55日（60日以内）
- **重大な発見**: omega下限5.0が狭すぎる可能性

**✅ Phase 2完了: omega下限をBoulder LPPLS準拠（2.0）に変更**
- commit: 実装完了
- 根拠:
  - **Boulder LPPLS標準**: omega ∈ **[2.0, 15.0]**（lppls.py:252）
  - **カスタムFCO（修正前）**: omega ∈ [5.0, 15.0]
  - **問題**: 全適格フィットがomega=5.0に張り付き → 最適値が5.0未満の可能性
  - **対策**: Boulder LPPLS標準範囲に統一
- 変更箇所:
  1. `lppl_optimizer.py`: グリッドサーチ範囲・bounds を 2.0-15.0 に変更（詳細コメント追加）
  2. `custom_fco_engine.py`: LPPL_BOUNDS・フィルタ範囲を 2.0-15.0 に変更（詳細コメント追加）
- **検証結果（1987年ブラックマンデー）**:
  - ✅ **omega境界張り付き解消**: 5.0000 → 4.3555-4.5842（境界から解放！）
  - ✅ 適格フィット数増加: 6/51 → 7/51（+1窓）
  - 🔺 DS-LPPLS Confidence: 11.76% → 13.73%（+1.97pt、目標30%には未達）
  - ⚠️ **新たな問題発見**: 全7適格フィットでtc=1.0100（下限張り付き）
  - ❌ 予測誤差: 91日（目標60日を超過）
- **結論**: omega問題は解決、しかしConfidence低迷の根本原因はO/D閾値の可能性

**🔍 Phase 2詳細分析（根本原因調査）**:

| 時期 | Confidence | omega状態 | tc状態 | O/Dフィルタ | 考察 |
|------|-----------|-----------|--------|------------|------|
| I130以前 | 43.14% | 5.0-15.0 | 不明 | 無効 | ベースライン（良好） |
| Phase 1 | 11.76% | 5.0-15.0 (全て5.0) | 全て1.0100 | **有効** | **-31.38pt大幅低下** |
| Phase 2 | 13.73% | **2.0-15.0 (4.36-4.58)** | 全て1.0100 | 有効 | omega解決、+1.97pt小幅改善 |

**根本原因の特定**:
- ✅ omega境界張り付き: Phase 2で完全解消（4.36-4.58、境界から解放）
- ⚠️ O/D閾値: Phase 1で-31.38pt低下、Phase 2でも未回復（-29.41pt残存）
- ⚠️ tc境界張り付き: Phase 1-2で継続（全フィットが1.0100）
- **結論**: O/D閾値（O>2.5, D>0.5）が1987年ブラックマンデーに対して過度に厳しい可能性

**tc境界張り付きについて**:
- tc=1.0100は「データ期間終了直後の予測」を意味（物理的には妥当）
- しかし全フィットが同じ境界値 = 最適化が制約されている
- 問題: tc < 1.0は過去予測（プロジェクト哲学に反する）
- 現状: tc下限を下げることは不可（未来予測の原則）
- 可能性: O/D閾値が厳しすぎるため、tcを未来に置かざるを得ない？

**カスタムFCO vs Boulder LPPLS比較表**:

| パラメータ | Boulder LPPLS | カスタムFCO（修正前） | カスタムFCO（修正後） |
|-----------|---------------|---------------------|---------------------|
| **beta** | 0.0 - 1.0 | 0.1 - 0.9 | 0.1 - 0.9 |
| **omega** | **2.0 - 15.0** | 5.0 - 15.0 | **2.0 - 15.0** ✅ |
| **O閾値** | > 2.5 | > 2.5 | > 2.5 |
| **D閾値** | > 0.5 | > 0.5 | > 0.5 |
| **tc範囲** | t2 ± (60~252日 or 期間の半分) | ⚠️ 無効化中 | ⚠️ 無効化中 |
| **境界張り付き** | なし | ⚠️ 無効化中 | ⚠️ 無効化中 |
...
**🎯 Phase 3の選択肢（ユーザー判断待ち）**:

**Option A: O/Dフィルタ一時無効化（影響確認）** ← 推奨
- 目的: O/D閾値がConfidence低迷の根本原因かを定量的に確認
- 方法: `ENABLE_TC_RANGE_CHECK = False` と同様に `ENABLE_OSCILLATIONS_DAMPING_CHECK` フラグを追加
- 期待結果: Confidence 13.73% → 43.14%に回復すればO/D閾値が原因と確定
- 利点: 根本原因の科学的確証、次のアクション判断材料
- 懸念: 一時的にBoulder LPPLS標準から逸脱

**Option B: O/D閾値の緩和**
- 目的: Boulder LPPLS標準より緩い閾値で1987年対応
- 方法: O > 2.0（2.5→2.0）、D > 0.3（0.5→0.3）に変更
- 期待結果: 適格フィット増加 → Confidence改善
- 利点: O/Dフィルタの本質は維持
- 懸念: 科学的根拠が薄い（Boulder LPPLS標準から逸脱）

**Option C: 1987年ブラックマンデーの特殊性調査**
- 目的: なぜ1987年がO/D基準を満たさないか科学的に理解
- 方法: 適格/不適格フィットのO/D値分布を詳細分析
- 期待結果: O/D閾値が不適切な理由の科学的説明
- 利点: 科学的理解の深化、適切な閾値設定の根拠
- 懸念: 時間がかかる、即効性なし

**推奨アプローチ**:
1. まずOption Aで根本原因を確定
2. Confidence回復を確認後、Option Cで科学的調査
3. 調査結果に基づきOption Bで適切な閾値を設定
...
### I133: 🔴 歴史的クラッシュ再現性テスト拡張（O/D閾値多面的検証）
**作成日**: 2025-10-13
**優先度**: 🔴 Critical
**担当**: カスタムFCO検証チーム
**状態**: 🚧 Phase 1完了、Phase 2移行

**目的**:
O/D閾値（O>2.5, D>0.5）の妥当性を複数の歴史的クラッシュで検証し、1987年単独ではない科学的根拠を確立する。

**背景（Issue I130関連）**:
- 1987年ブラックマンデー: O/D閾値実装で Confidence 43.14% → 11.76% 大幅低下
- omega拡大後も13.73%に留まる（-29.41pt、目標30%未達）
- O/D閾値が1987年に対して過度に厳しい可能性
- **ただし、O/D閾値はConfidenceの科学的基準であり、むやみに下げるべきではない**
- → 複数のクラッシュで検証し、総合的に判断する必要

**⚠️ Phase 1予備検証結果（2025-10-13、単一窓のみ、不十分）⚠️**

**暫定結果**: 単一窓（500日）検証では3クラッシュすべてでO/D閾値不合格

| クラッシュ | 窓構成 | R² | O | D | O/D判定 | 予測誤差 |
|-----------|--------|----|----|---|---------|---------|
| **1987年** | 51窓 | 0.9643 | - | - | ❌ | 20日 |
| **2000年** | **単一窓** | 0.9441 | 2.1681 | 0.7356 | ❌ | 302日 |
| **2008年** | **単一窓** | 0.7914 | 2.3893 | 0.0526 | ❌ | 51日 |

**⚠️ 重要な問題点（ユーザー指摘、2025-10-13）**:
- 2000年/2008年は**単一窓（500日）のみで検証**（計算時間短縮のため）
- **本番環境（126窓多重窓解析）と条件が異なり、科学的に不十分**
- さらに、**単一の分析基準日**（60日前のみ）でしか検証していない
- → **本番相当の条件（126窓×複数基準日）での再検証が必須**

**Option A/Bの提案は時期尚早**: まず本格検証を完了すべき

**詳細**: `workspace_for_claude/I133_od_threshold_validation_summary.md`（参考資料、結論は保留）

---

**実装計画（2025-10-13更新）**:

**Phase 1: 本番相当条件での歴史的クラッシュ検証** 🚧 再実装中
1. **2000年ドットコムバブル崩壊** 🚧 再実装必要
   - ❌ 既存実装: `workspace_for_claude/test_crash_2000_quick.py`（単一窓、不十分）
   - ✅ 必要な実装:
     - **126窓多重窓解析**（125-750日、5日刻み、FCO標準準拠）
     - **複数分析基準日**（クラッシュから30/45/60/90日前）
     - データ: NASDAQ Composite (NASDAQCOM)
     - クラッシュ日: 2000-03-10
     - 検証項目: 各基準日でのDS-LPPLS Confidence、O/D適合率

2. **2008年リーマンショック** 🚧 再実装必要
   - ❌ 既存実装: `workspace_for_claude/test_crash_2008_quick.py`（単一窓、不十分）
   - ✅ 必要な実装:
     - **126窓多重窓解析**（125-750日、5日刻み、FCO標準準拠）
     - **複数分析基準日**（クラッシュから30/45/60/90日前）
     - データ: NASDAQ100（SP500は2015年以降のみ）
     - クラッシュ日: 2008-09-15
     - 検証項目: 各基準日でのDS-LPPLS Confidence、O/D適合率

3. **1987年ブラックマンデー（参照）**
   - ✅ 既存: 51窓実装済み（`tests/custom_fco/test_phase2_multi_window.py`）
   - ⚠️ ただし基準日は42日前（101日前）のみ
   - 🚧 必要: 複数基準日（30/45/60/90日前）での追加検証

**実装方針**:
- `tests/custom_fco/test_crash_2000_dotcom_bubble.py` - 126窓×複数基準日対応
- `tests/custom_fco/test_crash_2008_lehman_shock.py` - 126窓×複数基準日対応
- プロット出力: `plots/reproducibility_tests/YYYY_crash_name_custom_fco_basis_XXXdays.png`
- 計算時間: 各クラッシュ×各基準日で約10分（許容範囲内）

**Phase 2: O/D閾値の統計的分析** ⏭️ Phase 1完了後
- 1987/2000/2008年多重窓データからO/D値を抽出・可視化
- O/D分布ヒストグラム、散布図作成
- 各基準日でのConfidence推移分析
- **⚠️ Phase 1で126窓×複数基準日でもConfidence低迷の場合のみ実施**

**Phase 3: 閾値調整の検討** ⏭️ Phase 2分析結果次第
- Option A: 閾値緩和（O>2.0, D>0.3）の検討
- Option B: O/D完全無効化の検討
- Option C: 適応的閾値の検討
- **⚠️ Phase 1でConfidence > 30%達成の場合は不要**

**Phase 4: 統合検証レポート作成** ⏭️ 最終段階
- 3クラッシュ×複数基準日の総合評価
- O/D閾値の妥当性に関する科学的結論
- Boulder LPPLS標準との差異説明（Issue I135参照）
- 最終的な推奨閾値の決定

**成功基準（修正版）**:
- ✅ **1987/2000/2008年で126窓×複数基準日での検証完了**
- ✅ **各基準日でDS-LPPLS Confidence > 30%達成を確認**
- ✅ プロット出力: `plots/reproducibility_tests/YYYY_crash_name_custom_fco_basis_XXXdays.png`
- ⚠️ すべての基準日で低Confidenceの場合: Phase 2/3でO/D閾値調整を検討

**関連Issue**:
- Issue I130: O/D閾値実装（✅ 完了、問題発見）
- Issue I129: カスタムFCO多重窓パラメータ整合性（✅ 解決済み）
- Issue I134: 再現性テストプロット整理（✅ 完了）
- **Issue I135: Boulder LPPLS標準の科学的根拠調査**（🆕 2025-10-13作成）

**実装ファイル**:
- `tests/custom_fco/test_crash_2000_dotcom_bubble.py` - 2000年126窓×複数基準日検証
- `tests/custom_fco/test_crash_2008_lehman_shock.py` - 2008年126窓×複数基準日検証
- `tests/custom_fco/test_phase2_multi_window.py` - 1987年参照実装（複数基準日追加予定）

---

### I134: 🟡 再現性テストプロット整理・命名規則統一
**作成日**: 2025-10-13
**優先度**: 🟡 High
**担当**: プロジェクト管理
**状態**: ✅ Phase 1-2完了、Phase 3完了

**目的**:
再現性テストのプロット出力を統一し、古いファイルを整理する。

**現状の問題**:
- プロット出力先が分散: `plots/crash_prediction/`, `plots/historical_crashes/`
- 命名規則が不統一: 日本語名混在、カスタムFCOのphaseナンバー等
- 古いファイルの蓄積: 削除基準不明
- CLAUDE.mdにプロット出力の必須化が明記されていない

**実装計画**:

**Phase 1: 命名規則統一** ⏭️
- ディレクトリ: `plots/reproducibility_tests/` に統一
- 命名規則（英語統一）:
  ```
  YYYY_crash_name_test_type.png
  例: 1987_black_monday_custom_fco_phase2.png
      2000_dotcom_bubble_custom_fco.png
      2008_lehman_crisis_custom_fco.png
  ```

**Phase 2: 既存ファイル整理** ⏭️
- 古いファイルを `plots/archives/YYYY-MM-DD/` に移動
- 最新の検証結果のみを `plots/reproducibility_tests/` に配置
- 整理基準:
  - カスタムFCO Phase 2完了版のみ残す
  - 旧LPPL実装の結果はアーカイブ
  - 日本語名ファイルはアーカイブ

**Phase 3: CLAUDE.md更新** ⏭️
- 再現性テストでのプロット出力を必須化
- 命名規則をドキュメント化
- 変更前必須テストにプロット確認を追加

**移行マッピング**:
| 現在のファイル | 新しいファイル | アクション |
|---------------|---------------|-----------|
| `plots/crash_prediction/1987_custom_fco_phase2_validation.png` | `plots/reproducibility_tests/1987_black_monday_custom_fco_phase2.png` | 移動・リネーム |
| `plots/crash_prediction/1987_custom_fco_phase2_best_fit_timeseries.png` | `plots/reproducibility_tests/1987_black_monday_custom_fco_phase2_timeseries.png` | 移動・リネーム |
| `plots/historical_crashes/1987_ブラックマンデー_analysis.png` | `plots/archives/2024-08-04/1987_ブラックマンデー_analysis.png` | アーカイブ |
| `plots/historical_crashes/2000_ドットコムバブル崩壊_analysis.png` | `plots/archives/2024-08-08/2000_ドットコムバブル崩壊_analysis.png` | アーカイブ |

**成功基準**:
- ✅ `plots/reproducibility_tests/` ディレクトリ作成・統一
- ✅ 全ての再現性テストが統一命名規則でプロット出力
- ✅ CLAUDE.mdに出力必須化を明記
- ✅ 古いファイルのアーカイブ完了

**関連Issue**:
- Issue I133: 歴史的クラッシュ再現性テスト拡張（🚧 並行実装）

**実装完了（2025-10-13）**:
- ✅ **Phase 1**: ディレクトリ作成・ファイル移動完了
  - `plots/reproducibility_tests/` 作成
  - 最新Phase 2ファイルをリネーム移動
- ✅ **Phase 2**: 古いファイルアーカイブ完了
  - `plots/archives/2024-08-04/`, `2024-08-08/`, `2024-10-11/`, `2024-10-12/` に整理
  - 日本語名ファイル・Phase 1ファイルをアーカイブ
- ✅ **Phase 3**: CLAUDE.md更新完了
  - 再現性テスト出力必須化を明記
  - 命名規則をドキュメント化

**実装ファイル**:
- `CLAUDE.md` (lines 417-426) - 再現性テスト出力必須化の追記完了 ✅
- `plots/reproducibility_tests/` - 新ディレクトリ作成完了 ✅
- `plots/archives/` - アーカイブディレクトリ整理完了 ✅

---

### I135: 🔵 Boulder LPPLS標準の科学的根拠調査（O/D閾値の起源）
**作成日**: 2025-10-13
**優先度**: 🔵 Medium（Issue I133 Phase 1並行実施）
**担当**: 科学的検証チーム
**状態**: 🆕 新規作成

**目的**:
Boulder LPPLS標準（O>2.5, D>0.5）の科学的根拠を調査し、閾値の妥当性を評価する。

**背景**:
- 本プロジェクトではBoulder LPPLSライブラリのデフォルト閾値を採用
- しかし、歴史的クラッシュ検証では過剰に厳格な可能性が示唆された
- **閾値の起源・科学的根拠を明確にする必要がある**（ユーザー指摘、2025-10-13）

**調査項目**:

**Phase 1: オンラインFCO解析結果の調査** 🚧 実施中
- Boulder LPPLS公式リポジトリ（GitHub）の調査
  - ドキュメント、論文引用、コミット履歴
  - O/D閾値決定の経緯・議論
- オンラインFCO実装サービスの調査
  - 公開されている解析結果のO/D条件
...
**Phase 2: 科学論文・文献の調査** 🚧 実施中
- Sornette論文シリーズでのO/D言及確認
  - `papers/extracted_texts/` 内の論文を再確認
  - O/D閾値に関する記述の検索
- Boulder LPPLS論文（あれば）の調査
  - パッケージ開発者の論文・技術レポート
  - 閾値決定の統計的根拠

**Phase 3: 実装コード・コメントの精査** 🚧 実施中
- Boulder LPPLSソースコード（lppls.py等）の詳細解析
  - O/D計算式の実装確認
  - コメント・docstringでの閾値説明
  - デフォルト値設定の意図
- 他のLPPL実装（あれば）との比較
  - PyPI、GitHub上の類似パッケージ
  - O/D閾値の違い・共通点
...
**調査方針**:
1. **Boulder LPPLS公式リポジトリ**: https://github.com/boulder-investment-technologies/lppls
2. **Sornette論文**: `papers/extracted_texts/sornette_2004_0301543v1_*.txt`
3. **オンライン検索**:
   - "LPPLS Oscillations Damping threshold"
   - "DS-LPPLS Confidence filter conditions"
   - "FCO bubble detection criteria"
4. **ソースコード**: `.local/lib/python3.10/site-packages/lppls/`（インストール済み）

**期待される結果**:
- ✅ O/D閾値の起源が明確になる（論文/経験則/統計分析）
- ✅ 閾値の適用可能性の限界を理解
- ⚠️ 科学的根拠が薄い場合: 本プロジェクト独自の閾値設定を正当化

**成功基準**:
- ✅ Boulder LPPLS標準の科学的根拠を文書化
- ✅ O/D閾値の決定方法を明確化
- ✅ 本プロジェクトでの閾値採用基準を提案
- ✅ 調査結果をIssue I133 Phase 4に反映

**関連Issue**:
- Issue I133: 歴史的クラッシュ再現性テスト拡張（🚧 主Issue、Phase 1実施中）
- Issue I130: O/D閾値実装（✅ 完了、問題発見）

**実装ファイル**:
- `workspace_for_claude/boulder_lppls_standard_investigation.md` - 調査結果まとめ
- `docs/scientific_validation/od_threshold_scientific_basis.md` - 最終レポート

**優先度について**:
- Issue I133 Phase 1（126窓×複数基準日検証）と**並行実施可能**
- Phase 1の結果次第で重要度が変動
  - Phase 1でConfidence達成 → 低優先度（参考資料）
  - Phase 1でConfidence低迷 → 高優先度（閾値調整の科学的根拠）
...
3. **統合検証**:
   - エントリーポイント: `python entry_points/main.py validate --crash 1987 --fco`
      - 本再現性テストでは、本番環境での解析の科学的な結果を保証することが必須です。
   - 基準: 全メトリクスクリア

4. **注記** :
   -oscillations および Damping 閾値によるフィルタリングを実装中（テストの基準も更新の可能性/2025/10/13現在）
...
3. **調査・解決方法の探索**: 問題の内容について、実装状況や関連するドキュメント、場合により根拠となる論文や公式ドキュメント・ライブラリ等を調査する。
4. **報告・確認**: 調査結果、および不明点、解決案・計画等について報告して確認する。さらに調査や解決方法の探索が必要である場合は、"2"のステップに戻る。
5. **計画・対応**: 作業計画を立て、実装・テスト・検証
6. **解決**: 完了後、Resolved Recentlyへ移動
7. **アーカイブ**: 1ヶ月後に別ファイルへ
