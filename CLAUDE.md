...
1. **プロジェクト理解の再構築**
   ...
   2. git log --oneline -20 で最近のコミット確認（進捗確認）
   3. docs/progress_management/CURRENT_ISSUES.md - アクティブな課題確認
   4. CLAUDE.md内の「現在進行中のタスク」セクションで現在のフェーズを確認
   ```
...
- **注意点**: 自分の場所が把握しにくくなるため、`pwd`コマンドをして、プログラムの実行前には自分の場所を把握。
...
### 1. 科学的正確性の維持
```
1. 論文再現機能（特にSornette論文のLPPLモデル実装）は、システムの科学的根幹であり、いかなる変更・改修においても絶対に破損させてはいけません。
2. 同様に、スタムFCOのテストでの過去の歴史的クラッシュの再現性テストの結果（※2025/10/13現在実装中）は、システムの科学的根幹であり、いかなる変更・改修においても絶対に破損させてはいけません。
3. 科学的な根拠づけや、解析のプロセス、実装内容が、後からコードを見た日とがだれでも理解できるように、科学的意味や単位、実装の目的などをコード内に適切に記述するようにしてください。これは、生成AIを利用した開発ではセッションが揮発的になるのに対して、むやみにコードおよび解析方法がむやみに変更されないようにするためです。
```
...
- あくまで、教育・研究目的のサービスとして、科学的な手法に基づいた解析結果およびその可視化を提供するサービスとして提供する
- 「クラッシュ予測」等の表現を使用しない。変わりに、臨界点などの表現を使う（発見しだい逐次置き換え）。
- 数値データのみ提供（投資判断を含まない）
- ユーザーが追加分析を行うことは可能な設計にする
詳細: docs/service_commercialization/legally_compliant_service_specification.md
```

### 3. コードを正とする原則 📝

**⚠️ CRITICAL: ドキュメントとコードの差異が発生した場合の対応原則**

```
1. コードが正しく動作している限り、コードを正（真実）とする
2. ドキュメント（CLAUDE.md、設計書等）をコードの実装に合わせて更新する
3. コードが意図通りに動作していない場合のみ、コードを修正する
```

**理由**:
- 生成AIセッションの揮発性により、ドキュメントが更新されずコードのみ変更されるケースがある
- 実際に動作しているコードが「真実」であり、ドキュメントはその説明にすぎない
- 逆（ドキュメントに合わせてコードを変更）は、動作しているシステムを破壊するリスクがある

**適用例**（Issue I131で発見）:
- CLAUDE.md記載: omega範囲 [5.0, 10.0]
- 実際のコード: omega範囲 [5.0, 15.0]（Issue I129で意図的に変更済み）
- **対応**: CLAUDE.mdを15.0に更新（コードは変更しない）

**コード整理の例外**:
- **アーキテクチャのシンプル化**: 管理しやすくし、科学的手法を明確にするため、コードを整理することがある
  - 重複コードの統合
  - 複雑な実装の簡潔化（科学的正確性を維持しつつ）
  - 命名の改善・コメントの充実化
- **前提**: コード整理時も科学的検証テストの合格を必須とする

**不明点の逐次確認（重要）**:
- **特に科学的根拠に関わる点**: 実装前に必ずユーザーに確認すること
- **確認すべき項目**:
  - パラメータ範囲の科学的妥当性（論文との整合性）
  - フィルタリング条件の追加・変更
  - 最適化アルゴリズムの選択
  - 数式実装の解釈が不明確な場合
- **原則**: 推測での実装は禁止。不明点は必ずユーザーに確認してから実装

**例外**:
- コードにバグがある場合（科学的検証テストが失敗する場合）
- コードが論文再現性を破壊している場合
- セキュリティ脆弱性がある場合

これらの例外ケースでは、コードを修正し、テストを実行して妥当性を確認すること。
...
**⚠️ CRITICAL: Issue I132承認（2025-10-13）による新方針**

### 🔄 **git commit中心の進捗管理**

**進捗状況の確認方法**:
1. **git log**: `git log --oneline -20` で最近の作業内容を確認
2. **Issue管理**: `docs/progress_management/CURRENT_ISSUES.md` で詳細なタスク追跡・調査結果
3. **CLAUDE.md**: このファイル内の「現在進行中のタスク」セクション（下記）で概要を把握

**CURRENT_PROGRESS.mdは廃止**:
- ✅ **廃止決定**: 今後の更新は停止
- ✅ **代替手段**: git commitメッセージで進捗を記録
- ✅ **長期的な進捗履歴**: git log + Issue管理で追跡

**実装時の原則**:
- 各作業完了時に詳細なcommitメッセージを残す（日本語可）
- 科学的根拠・実装意図をcommitメッセージに記載
- Issue番号を必ずcommitメッセージに含める（例: `✅ Issue I130完了: Oscillations/Damping閾値追加`）

### 🌿 **ブランチ戦略: シンプルなdev/main構成**

**基本方針**:
- ✅ **main**: 安定版（本番相当、慎重にマージ）
- ✅ **dev**: 開発版（Claude Codeの作業対象、積極的な実装）
- ⚠️ **feature/*ブランチは使用しない**: 管理負担が大きいため、シンプルな構成を採用
...
# コミット（Issue番号付き）
git add .
git commit -m "✅ Issue I130: Oscillations/Damping閾値追加

- custom_fco_engine.pyにO > 2.5, D > 0.5を追加
- Boulder LPPLS FCOとの整合性向上
- 検証テスト: 1987年ブラックマンデー適格フィット数測定
...
- **Issue I130**: Boulder LPPLS FCOフィルタリング調査（🔄 実装中）
  - Oscillations/Damping閾値追加 + 過剰フィルタリング緩和
- **Issue I131**: CLAUDE.md整理タスク（🔄 進行中）
- **Issue I132**: 進捗管理方法の検討（✅ 承認済み、実装中）
- **Issue I133**: 再現性テストのCI/CD化（📋 計画中、Phase 3後に実施）
...
└── CURRENT_ISSUES.md      # アクティブな課題（詳細なタスク追跡）

git log --oneline -20      # 最近の進捗確認（commit履歴）
CLAUDE.md内の「現在進行中のタスク」セクション  # 現在のフェーズ概要
```
**重要**: タスク開始時は必ず git log と CURRENT_ISSUES.md で現状を確認すること。
**注意**: CURRENT_PROGRESS.mdは廃止（Issue I132承認、2025-10-13）
...
2. git log --oneline -20 - 最近の進捗確認
3. docs/progress_management/CURRENT_ISSUES.md - アクティブな課題
4. CLAUDE.md内の「現在進行中のタスク」セクション - 現在のフェーズ確認
5. システム構造の最新状況確認 - アクティブなコード特定
```
...
**最終更新**: 2025-10-13 (Issue I132承認: git commit中心の進捗管理・dev/main ブランチ戦略・コードを正とする原則拡張)
