...
│
├── 🟢 scheduled_nasdaq_analysis.py   # NASDAQ解析（アクティブ）
├── 🟢 scheduled_aapl_analysis.py     # 個別株解析（アクティブ）  
...
**🟢 主要エントリポイント（現在稼働中）**：
- `scheduled_nasdaq_analysis.py` - NASDAQ解析（FRED API）
- `scheduled_aapl_analysis.py` - 個別株解析（Alpha Vantage API）
- `examples/basic_analysis.py` - デモ・テスト用統合システム
- `src/web_interface/symbol_visualization_dashboard.py` - Streamlitダッシュボード
...
```
実際の動作フロー：
Entry Points → Data Client → LPPL Fitting → Database → Dashboard
     ↓             ↓           ↓           ↓         ↓
scheduled_*.py  unified    fitter.py    SQLite   dashboard.py
   (Root)      (src/data)  (src/fit)   (src/db)  (src/web)
```
