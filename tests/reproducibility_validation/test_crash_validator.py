...
class TestCrashValidator(unittest.TestCase):
    ...
    #     self.assertAlmostEqual(result.parameters['beta'], m, delta=0.05)
    ...
    #     # 対数周期関数に従うデータを生成
    #     dt = tc - t
    #     prices = A + B * np.power(dt, beta) * (1 + C * np.cos(omega * np.log(dt) + phi))
    ...
    #     # 結果の検証
    #     self.assertTrue(result.success)
    #     self.assertAlmostEqual(result.parameters['beta'], beta, delta=0.05)
