#!/usr/bin/env python3
"""
1987年ブラックマンデー - カスタムFCO多重窓検証（複数基準日版）(Issue I133)

目的: O/D閾値（O>2.5, D>0.5）の妥当性を1987年ブラックマンデーで検証
...
注: test_phase2_multi_window.pyは単一基準日（101日前）のみのため、
    複数基準日対応版として本スクリプトを作成
"""
...
print("=" * 80)
print("1987年ブラックマンデー - カスタムFCO多重窓検証（複数基準日版）(Issue I133)")
print("=" * 80)
print()
...
# 新: n_tries=25 でランダム初期値生成 25回（1/20の計算量）
...
if successful_bases >= len(results_by_basis) * 0.5:
    print("🎉 1987年ブラックマンデー検証成功")
    print("✅ O/D閾値が1987年ブラックマンデーで妥当であることを確認")
    overall_success = True
else:
    print("❌ 1987年ブラックマンデー検証失敗")
    print("⚠️ 複数の基準日でConfidence目標未達")
    overall_success = False

print()
print("=" * 80)
print("1987年ブラックマンデー検証完了")
print("=" * 80)
