#!/usr/bin/env python3
"""
2000年ドットコムバブル崩壊 - カスタムFCO多重窓検証 (Issue I133)

目的: O/D閾値（O>2.5, D>0.5）の妥当性を2000年ドットコムバブルで検証
...
# データ読み込み（既存のフルキャッシュを使用）
cache_path = Path("data/market_data/cache/full/NASDAQCOM_full.parquet")

if not cache_path.exists():
    print(f"❌ エラー: キャッシュファイルが見つかりません: {cache_path}")
    print("まず全銘柄解析を実行してキャッシュを作成してください:")
    print("  python entry_points/main.py analyze NASDAQCOM")
    sys.exit(1)

# フルデータ読み込み
df = pd.read_parquet(cache_path)

# 必要な期間のみフィルタリング（1995-2002年）
data_start = datetime(1995, 1, 1)
data_end = datetime(2002, 12, 31)
...
# 2000年ドットコムバブル崩壊の設定
# NASDAQピーク: 2000-03-10（5048.62ポイント）
crash_date = datetime(2000, 3, 10)

# 複数分析基準日（クラッシュから30/45/60/90日前）
basis_days_before = [30, 45, 60, 90]

print("=" * 80)
print("2000年ドットコムバブル崩壊 - カスタムFCO多重窓検証 (Issue I133)")
print("=" * 80)
print()
print(f"データ期間: {df.index[0].date()} ~ {df.index[-1].date()}")
print(f"バブル崩壊日（NASDAQピーク）: {crash_date.date()}")
...
if successful_bases >= len(results_by_basis) * 0.5:
    print("🎉 2000年ドットコムバブル検証成功")
    print("✅ O/D閾値が2000年ドットコムバブルで妥当であることを確認")
    overall_success = True
else:
    print("❌ 2000年ドットコムバブル検証失敗")
    print("⚠️ 複数の基準日でConfidence目標未達")
    overall_success = False

print()
print("=" * 80)
print("2000年ドットコムバブル検証完了")
print("=" * 80)
